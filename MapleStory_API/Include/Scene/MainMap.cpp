...
bool CMainMap::Init()
{
	...
	CPlayer* Player = CreateObject<CPlayer>("Player", Vector2(100.f, 1150.f));

	if (CSceneManager::GetInst()->GetName() == "TutorialMap")
	{
		Player->SetPos(100.f, 1050.f);
	}
	...
	CPotal* Potal = CreateObject<CPotal>("MaintoStage1", Vector2(1500.f, 906.f));
...
{
	GetSceneResource()->LoadSound("BGM", true, "MainMap", "TempleInTheMirror.mp3");
	GetSceneResource()->SoundPlay("MainMap");
